namespace IntegrationV2
{
	...
	public class SyncJobScheduler : ISyncJobScheduler
	{
		...
		private void CreateProcessJob(UserConnection uc, string syncJobName, string processName,
			...
			RemoveProcessJob(syncJobName, uc);
			...
			if (period == 0) {
				...
				if (useNewMeetingIntegration && processName == ExchangeConsts.ActivitySyncProcessName) {
					...
					_appSchedulerWraper.ScheduleImmediateProcessJob(syncJobName, SyncJobGroupName, processName,
						uc.Workspace.Name, currentUser.Name, syncJobParameters);
				}
			} else {
				_log.Error($"ScheduleMinutelyJob called: CurrentUser {currentUser.Name}, SyncJobName {syncJobName}");
				_appSchedulerWraper.ScheduleMinutelyJob(syncJobName, SyncJobGroupName, processName,
					uc.Workspace.Name, currentUser.Name, period, syncJobParameters);
			}
		}
