namespace Terrasoft.IntegrationV2.Utils
{
	...
	internal class ListenerUtils
	{
		...
		private static readonly Lazy<IFeatureUtilities> _featureUtilities = new Lazy<IFeatureUtilities>(() => ClassFactory.Get<IFeatureUtilities>());

		private static IFeatureUtilities FeatureUtilities {
			get {
		...
		public static bool GetIsFeatureEnabled(string code) {
			return Features.GetIsEnabled(code);
		}
		...
		/// <returns><c>True</c> if feature enabled, otherwise returns false.</returns>
		public static bool GetIsFeatureEnabled(string code, Guid sysAdminUnitId) {
			return Features.GetIsEnabled(code, sysAdminUnitId);
		}
		...
		public static bool GetIsFeatureDisabled(string code) {
			return !GetIsFeatureEnabled(code);
		}
		...
		/// <param name="sysAdminUnitId">SysAdminUnit unique identifier.</param>
		/// <returns><c>True</c> if feature disabled, otherwise returns false.</returns>
		public static bool GetIsFeatureDisabled(string code, Guid sysAdminUnitId) {
			return !GetIsFeatureEnabled(code, sysAdminUnitId);
		}

		/// <summary>
		/// Returns is feature <paramref name="code"/> enabled for any user.
		/// </summary>
		/// <param name="uc"><see cref="UserConnection"/> instnace.</param>
		/// <param name="code">Feature code.</param>
		/// <returns><c>True</c> if feature enabled for any user, otherwise returns false.</returns>
		public static bool GetIsFeatureEnabledForAnyUser(UserConnection uc, string code) {
			return FeatureUtilities.GetIsFeatureEnabledForAnyUser(uc, code);
		}

		/// <summary>
		/// Returns is feature <paramref name="code"/> enabled for all users.
		/// </summary>
		/// <param name="uc"><see cref="UserConnection"/> instnace.</param>
		/// <param name="code">Feature code.</param>
		/// <returns><c>True</c> if feature enabled for all users, otherwise returns false.</returns>
		public static bool GetIsFeatureEnabledForAll(UserConnection uc, string code) {
		...
		public static void TryDoListenerAction(Action action, string senderEmailAddress, UserConnection uc) {
			try {
				...
				if (GetIsFeatureDisabled("OldEmailIntegration") && GetIsFeatureEnabled("HandleIntNetworkError")) {
